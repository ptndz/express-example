...
import {
  createFile,
  getFileByCheckSumAnhSize,
  getFiles,
} from "../services/file";
import { getUser } from "../services/user";
import { IResponse, Media, Pagination } from "../types";
import { dateNow, getExtensionFromMimeType, removeKeyObject } from "../utils";
    ...
    try {
      ...
      await Promise.all(
        files.map(async (file: Express.Multer.File) => {
          ...
          const existingFile = await getFileByCheckSumAnhSize(
            checksum,
            String(file.size)
          );
          if (existingFile) {
            ...
            dataFile2.url = encodeURI(
              `${APP_URL}/${dataFile2.path}/${dataFile2.name}`
            );
            media.push(dataFile);
            console.log(
          ...
          const filenameRemoveVietNam = removeVietNam(filename)
            .split(" ")
            .join("");
          ...
          const pathYearMonth = `files/${typeDir}/${dateNow().yyyy}/${
            dateNow().mm
          }/${dateNow().dd}`;
          ...
          const data: any = {
            ...
            type: getExtensionFromMimeType(file.mimetype),
            user: user,
            checksum: checksum,
          };
